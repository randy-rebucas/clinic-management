...
import { processRecurringAppointments } from '@/lib/automations/recurring-appointments';
...
export async function GET(request: NextRequest) {
  ...
  try {
    const result = await processRecurringAppointments();

    return NextResponse.json({
      success: true,
      message: 'Recurring appointments processed',
      data: result,
    });
  } catch (error: any) {
    console.error('Error processing recurring appointments:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to process recurring appointments' },
      { status: 500 }
    );
    }
}
