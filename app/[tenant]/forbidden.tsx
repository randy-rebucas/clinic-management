...
export default function ForbiddenPage() {
  // Removed dict state, using fallback text directly
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const profileData = await fetch('/api/auth/profile', { credentials: 'include' }).then(res => res.json());
      ...
      <div className="max-w-lg w-full bg-white rounded-lg shadow-xl p-8 text-center">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
          <p className="text-sm text-yellow-800">
            <strong>Security Notice: This access attempt has been logged for security purposes.</strong>
          </p>
        </div>
        ...
        <div className="space-y-3">
          {userInfo.tenantSlug && (
            ...
            >
              {`Go to My Store (${userInfo.tenantSlug})`}
            </button>
          )}
          <Link
            href="/default/en"
