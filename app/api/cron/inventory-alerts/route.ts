...
import { processInventoryAlerts } from '@/lib/automations/inventory-alerts';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Inventory alerts processed',
      data: result,
    });
  } catch (error: any) {
    console.error('Error processing inventory alerts:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to process inventory alerts' },
      { status: 500 }
    );
  }
}
