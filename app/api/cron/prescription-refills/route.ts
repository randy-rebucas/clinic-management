...
import { processRefillReminders } from '@/lib/automations/prescription-refills';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Prescription refill reminders processed',
      data: result,
    });
  } catch (error: any) {
    console.error('Error processing prescription refill reminders:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to process prescription refill reminders' },
      { status: 500 }
    );
  }
}
