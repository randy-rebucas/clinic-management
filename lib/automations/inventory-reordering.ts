...
async function sendReorderNotification(
  ...
  try {
    ...
    if (firstAdmin && (firstAdmin as any).email) {
...
export async function processInventoryReordering(
  ...
  try {
    return await run(tenantId, async () => {
      ...
      const lowStockItems = await listInventoryItems({
        OR: [
      ...
      // Get expiring items (within 90 days, still in stock)
      const ninetyDaysFromNow = new Date();
      ninetyDaysFromNow.setDate(ninetyDaysFromNow.getDate() + 90);

      const expiringItems = await listInventoryItems({
        expiryDate: { not: null, lte: ninetyDaysFromNow, gte: new Date() },
        quantity: { gt: 0 },
      } as any);
      ...
      for (const item of expiringItems) {
        if (!allItems.find((i: any) => i.id === (item as any).id)) {
          allItems.push(item);
        }
      }
